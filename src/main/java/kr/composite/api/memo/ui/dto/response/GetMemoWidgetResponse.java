...
@Schema(description = "메모 위젯 조회 응답")
public record GetMemoWidgetResponse(
    ...
    public static GetMemoWidgetResponse from(MemoWidgetResponse response) {
        return new GetMemoWidgetResponse(
                response.id(),
                response.widgetId(),
                response.title(),
                response.content(),
